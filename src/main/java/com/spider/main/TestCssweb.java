		...
		for (;;) {
			try{
				...
				while (out != 1) {
					...
					System.out.println();
					doc.select(refcss+" "+urlcss).forEach(System.out::println);
					System.err.println(doc.selectFirst(refcss+" "+urlcss).attr("abs:href"));
					...
					if(!"".equals(reftitlecss)){
						...
						title=doc.select(refcss+" "+reftitlecss).attr("title");
							if(null==title||"".equals(title)){
								title=doc.select(refcss+" "+reftitlecss).attr("alt");
								if(null==title||"".equals(title)){
									title=doc.select(refcss+" "+reftitlecss).text();
								}
							}
						}
					...
					try{
						...
						System.out.println(doc.selectFirst(refcss+" "+reftimecss).attr("href"));
						System.out.println(PtimePick.getTime(doc.selectFirst(refcss+" "+reftimecss).attr("href")));
					}
...
//				}
				out = 0;
				String conurl = doc.selectFirst(refcss+" "+urlcss).attr("abs:href");
				driver.get(conurl);
				WdUtils.waitPageLoad(driver,1000);
